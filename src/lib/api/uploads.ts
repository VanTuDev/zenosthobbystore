...
/** Admin-only bank transaction screenshot upload. */
export async function uploadPaymentProofImage(file: File): Promise<UploadImageResponse> {
  const form = new FormData();
  form.append("file", file);

  const res = await fetch(`${API_BASE_URL}/uploads/payment-proof`, {
    method: "POST",
    credentials: "include",
    headers: getAuthHeaders(),
    body: form,
...
export async function uploadContactImage(file: File): Promise<UploadImageResponse> {
  ...
  const res = await fetch(`${API_BASE_URL}/uploads/contact-image`, {
    method: "POST",
    credentials: "include",
    body: form,
  });
